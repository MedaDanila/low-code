...
export function generatedApiDevServer(): Plugin {
  let apiDatabase: AppDatabase | null = null

  async function loadApiDatabase(): Promise<AppDatabase> {
    const { readSqliteDatabase } = await import('./sqliteDatabase.js')
    apiDatabase = readSqliteDatabase(createSeedDatabase)
    return apiDatabase
  }
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          if (url.pathname === '/api/v1/__runtime/snapshot') {
            ...
            const { sqliteDatabasePath, writeSqliteDatabase } = await import('./sqliteDatabase.js')
            apiDatabase = writeSqliteDatabase(snapshot)
            sendJson(res, 200, {
              ok: true,
              storage: 'sqlite',
          ...
          if (url.pathname === '/api/v1/__runtime/database') {
            if (req.method === 'GET') {
              apiDatabase = await loadApiDatabase()
              sendJson(res, 200, apiDatabase)
              return
            }
            ...
            if (req.method === 'PUT') {
              ...
              const { writeSqliteDatabase } = await import('./sqliteDatabase.js')
              apiDatabase = writeSqliteDatabase(snapshot)
              sendJson(res, 200, apiDatabase)
              return
            }

            sendJson(res, 405, { error: 'Метод не поддерживается', allowed: ['GET', 'PUT'] })
            return
          }

          if (url.pathname === '/api/v1/__runtime/database/reset') {
            if (req.method !== 'POST') {
              sendJson(res, 405, { error: 'Метод не поддерживается', allowed: ['POST'] })
              return
            }

            const { resetSqliteDatabase } = await import('./sqliteDatabase.js')
            apiDatabase = resetSqliteDatabase(createSeedDatabase)
            sendJson(res, 200, apiDatabase)
            return
          }
